...
下一批功能将围绕“专业海报工作区”和“可解释布光”展开：

- **海报模式工作区**：独立入口，进入后画板占据主体，参数、流程、构图、布光与结果统一收纳到右侧检视器。
- **最终成图尺寸控制**：支持比例 + 长边分辨率、仅比例联动宽高、自由尺寸三种模式，画布尺寸即最终输出尺寸。
- **相机与布光升级**：三视图联动、动态光源、光源覆盖范围可视化，并提供 Three.js 3D 预览辅助理解空间关系。
- **更稳定的 AI 编排**：分析、灯光细化与结果归一化分层处理，降低提示词语义冲突，提高 FLUX / SD / Midjourney 可理解性。
- **模板与专业模式基础**：为后续模板导入导出、Pro 菜单化比例库与常见纸张比例做数据结构准备。
